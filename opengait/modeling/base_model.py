...
from data.dataset import DataSet, DataSet_distr
import data.sampler as Samplers
from utils import Odict, mkdir, ddp_all_gather
from utils import get_valid_args, is_list, is_dict, np2var, ts2np, list2var, get_attr_from
from evaluation import evaluation as eval_functions
from utils import NoOp
from utils import get_msg_mgr
from utils.clip_grad import ClipGrad
__all__ = ['BaseModel']
...
class BaseModel(MetaModel, nn.Module):
    ...
    def __init__(self, cfgs, local_rank, training):
        ...
        if 'num_train' in cfgs['data_cfg']:
            self.save_path = osp.join('output/', self.cfgs['data_cfg']['dataset_name'],
                                      self.cfgs['model_cfg']['model'],
                                      self.engine_cfg['save_name'] + '_' + str(self.cfgs['data_cfg']['num_train']))
        else:
            self.save_path = osp.join('output/', self.cfgs['data_cfg']['dataset_name'], self.cfgs['model_cfg']['model'],
        ...
        self.use_dis = False  # gxd
        if not training or self.engine_cfg['with_test']:
            if 'num_distr' in cfgs['data_cfg'].keys() and self.cfgs['data_cfg']['dataset_name'] == 'GREW':
                self.test_loader = self.get_loader_DIS(cfgs['data_cfg'], train=False)
                self.use_dis = True
            else:
                # pass
        ...
        self.to(device=torch.device("cuda", self.device))   # gxd


        if training:
            self.loss_aggregator = LossAggregator(cfgs['loss_cfg'], self.device)
    ...
    def get_loader(self, data_cfg, train=True):
        ...
        self.list_all = label_list, types_list, views_list
    ...
    #    distractor loader
    def get_loader_DIS(self, data_cfg, train=True):
        sampler_cfg = self.cfgs['trainer_cfg']['sampler'] if train else self.cfgs['evaluator_cfg']['sampler']
        dataset = DataSet(data_cfg, train)
        ...
        print('get distractor by get_loader!')
        from torch.utils.data import ConcatDataset
        dataset_distr = DataSet_distr(data_cfg)

        label_list.extend(dataset_distr.label_list)
        types_list.extend(dataset_distr.types_list)
        views_list.extend(dataset_distr.views_list)
        label_set.extend(dataset_distr.label_set)
        self.list_all = label_list, types_list, views_list
        dataset = ConcatDataset([dataset, dataset_distr])
        # dataset = ConcatDataset([dataset_distr, dataset])
        ...
        loader = tordata.DataLoader(
            dataset=dataset,
            batch_sampler=sampler,
            collate_fn=CollateFn(label_set, sampler_cfg),
            num_workers=data_cfg['num_workers'])
        return loader
    ...
    def run_test(model):
        ...
        if rank == 0:
            ...
            if model.cfgs["data_cfg"]["dataset_name"] == 'GREW':
